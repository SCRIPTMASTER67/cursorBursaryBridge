/**
 * DEVELOPMENT DEMO DATA — NEVER FOR PRODUCTION.
 *
 * Every funder, funding programme, student and application below is INVENTED.
 * They exist so a developer has something to click through, and they must
 * never reach a real deployment: a student who sees one of these would be
 * looking at a bursary that does not exist.
 *
 * Production seeding is `prisma/seed.ts`, which creates reference data only.
 * This file refuses to run unless it is explicitly asked for, and refuses
 * outright in production.
 *
 * Run it with:  SEED_DEMO_DATA=yes npm run db:seed:demo
 */
import '../lib/load-env';
import {
  PrismaClient,
 ...
 * The guard.
 *
 * Two conditions, both required: not production, and asked for by name. A
 * default-off switch is the only thing that reliably keeps invented bursaries
 * out of a real database.
 */
function refuseUnlessDevelopment() {
  if (process.env.NODE_ENV === 'production') {
    console.error(
      'Refusing to run: this seed creates invented bursaries and NODE_ENV is production.',
    );
    process.exit(1);
  }
  if (process.env.SEED_DEMO_DATA !== 'yes') {
    console.error(
      'Refusing to run: this seed creates invented bursaries, organisations and\n' +
        'applications that must never reach a real database.\n\n' +
        'If you are a developer and want demo data locally:\n' +
        '  SEED_DEMO_DATA=yes npm run db:seed:demo\n\n' +
        'For a production database use `npm run db:seed`, which creates the\n' +
        'institution and course catalogue and nothing else.',
    );
    process.exit(1);
  }
}
...
async function main() {
  ...
  console.log('Seeding DEMO data (invented funders and bursaries)…');
  ...
  console.log('\nEVERY funder and bursary created above is invented. Do not deploy this data.');
