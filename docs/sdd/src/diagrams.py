...
# ---------------------------------------------------------------------------
def sequence(path, lifelines, messages, font_px=16, box_w=190, box_h=62,
             gap_y=74, pad=26, top_gap=40, label_px=None):
    """
    A UML sequence diagram.

    lifelines  [label]                       drawn left to right
    messages   [(from_i, to_i, label)] or [(from_i, to_i, label, kind)]
                                             from_i == to_i draws a self-call.
                                             kind is 'call' (solid) or 'return'
                                             (dashed); when omitted, a label
                                             ending in ')' is taken to be a call.

    Lifelines are spaced so that the longest message label fits between the two
    columns it joins, which is what keeps the text off the arrowheads.
    """
    import math
    label_px = label_px or font_px - 1
    n = len(lifelines)

    probe = Image.new("RGB", (10, 10))
    pd = ImageDraw.Draw(probe)
    f_life = _font(font_px)
    f_msg = _font(label_px)

    # Column pitch: wide enough for the boxes and for every label that spans a
    # single column step.
    pitch = box_w + 46
    for msg in messages:
        src, dst, label = msg[0], msg[1], msg[2]
        span = abs(dst - src)
        if span == 0:
            # A self-call writes its label to the right of the loop, so it needs
            # a column's worth of room too, or it runs into the next lifeline.
            need = _text_size(pd, label, f_msg)[0] / S + 110
        else:
            need = (_text_size(pd, label, f_msg)[0] / S + 34) / span
        pitch = max(pitch, need)
    pitch = int(pitch)

    width = int(pad * 2 + box_w + pitch * (n - 1))
    height = int(pad * 2 + box_h + top_gap + gap_y * len(messages) + 30)

    img = Image.new("RGB", (width * S, height * S), "white")
    d = ImageDraw.Draw(img)
    lw = max(1, int(1.3 * S))

    xs = [pad + box_w // 2 + pitch * i for i in range(n)]
    box_top, box_bot = pad, pad + box_h
    life_bot = height - pad

    for i, label in enumerate(lifelines):
        cx = xs[i] * S
        d.rectangle([cx - box_w * S // 2, box_top * S,
                     cx + box_w * S // 2, box_bot * S],
                    outline="black", fill="white", width=lw)
        lines = _wrap(d, label, f_life, int(box_w * S * 0.86))
        _, lh = _text_size(d, "Xg", f_life)
        gap = int(lh * 1.32)
        top = (box_top + box_h / 2) * S - (len(lines) - 1) * gap / 2 - lh / 2
        for k, ln in enumerate(lines):
            tw, _ = _text_size(d, ln, f_life)
            d.text((cx - tw / 2, top + k * gap), ln, font=f_life, fill="black")

        # dashed lifeline
        y = box_bot * S
        dash = int(7 * S)
        while y < life_bot * S:
            d.line([cx, y, cx, min(y + dash, life_bot * S)], fill="black", width=lw)
            y += dash * 2

    def arrow(x1, y, x2, dashed):
        head = 9 * S
        if dashed:
            step, x = int(6 * S), x1
            direction = 1 if x2 > x1 else -1
            while (x - x2) * direction < 0:
                nxt = x + step * direction
                if (nxt - x2) * direction > 0:
                    nxt = x2
                d.line([x, y, nxt, y], fill="black", width=lw)
                x = nxt + step * direction
        else:
            d.line([x1, y, x2, y], fill="black", width=lw)
        ang = 0 if x2 > x1 else math.pi
        for off in (2.6, -2.6):
            d.line([x2, y,
                    x2 + head * math.cos(ang + off),
                    y + head * math.sin(ang + off)], fill="black", width=lw)

    y = box_bot + top_gap
    for msg in messages:
        src, dst, label = msg[0], msg[1], msg[2]
        kind = msg[3] if len(msg) > 3 else None
        dashed = (kind == "return") if kind else not label.rstrip().endswith(")")
        if src == dst:
            # self-call: out, down, back
            cx = xs[src] * S
            w = int(52 * S)
            h = int(26 * S)
            d.line([cx, y * S, cx + w, y * S], fill="black", width=lw)
            d.line([cx + w, y * S, cx + w, y * S + h], fill="black", width=lw)
            arrow(cx + w, y * S + h, cx + int(4 * S), dashed)
            tw, th = _text_size(d, label, f_msg)
            d.text((cx + w + int(10 * S), y * S - th / 2), label,
                   font=f_msg, fill="black")
        else:
            x1, x2 = xs[src] * S, xs[dst] * S
            inset = int(4 * S) * (1 if x2 > x1 else -1)
            arrow(x1 + inset, y * S, x2 - inset, dashed)
            tw, th = _text_size(d, label, f_msg)
            d.text(((x1 + x2) / 2 - tw / 2, y * S - th - int(7 * S)), label,
                   font=f_msg, fill="black")
        y += gap_y
